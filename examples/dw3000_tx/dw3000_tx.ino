...
uint8_t txPayload[TX_PAYLOAD_LEN];       // Small payload buffer with counter + pattern
uint32_t frameCounter = 0;

void configureRadioCommon();
void buildPayload(uint32_t counter, uint8_t* payload, size_t len);
bool waitForTxDone();
...
void loop() {
  buildPayload(frameCounter, txPayload, TX_PAYLOAD_LEN);

  // Clear stale status bits before arming a new transmission.
  DW3000.clearSystemStatus();

  // Copy the payload into the TX buffer starting at offset 0.
  DW3000.pullLEDHigh(2);
  DW3000.writeTXBuffer(txPayload, TX_PAYLOAD_LEN, 0);
...
void buildPayload(uint32_t counter, uint8_t* payload, size_t len) {
  if (payload == nullptr || len < TX_PAYLOAD_LEN) {
    return;
  }

  // 32-bit frame counter in little-endian order.

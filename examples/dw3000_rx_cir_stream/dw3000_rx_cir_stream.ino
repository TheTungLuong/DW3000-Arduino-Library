...
void loop() {
  ...
  if (rx_status == 1) {
    ...
    // Clear out the accumulator so the next CIR snapshot starts from fresh data
    // instead of re-reading leftover entries.
    flushAccumulator();
...
void flushAccumulator() {
  ...
  // Zero ACC_MEM in 4-byte words so that the next frame's accumulation starts clean.
  // The write() helper supports multi-byte writes via data_len, so iterate across
  // the window we read for CIR extraction.
  for (size_t offset = 0; offset < totalBytes; offset += sizeof(uint32_t)) {
    DW3000.write(ACC_MEM_REG, firstByteOffset + offset, 0, sizeof(uint32_t));
  }
}
